...
#SBATCH --output=../logs/run_data.log
...
do
	...
	do
		...
		MODEL_PATH=$([ "$DATA_TYPE" = "uos" ] && echo "${SAVE_DIR}/width_${WIDTH}_depth_${DEPTH}_nonlinear_depth_${NONLINEAR_DEPTH}_${INIT}_init_${DATA_TYPE}_data_${NUM_CLASSES}_classes_rank_${RANK}_angle_${ANGLE}_${ACTIVATION}_activation_seed_${SEED}" || echo "${SAVE_DIR}/width_${WIDTH}_depth_${DEPTH}_nonlinear_depth_${NONLINEAR_DEPTH}_${INIT}_init_${DATA_TYPE}_data_${NUM_CLASSES}_classes_${ACTIVATION}_activation_seed_${SEED}")
